...
script_path=~/jmeter-workshop-032023/Workshop-2-5-Cli_DistributedLoad/jmeter
result_path=~/jmeter-workshop-032023/Workshop-2-5-Cli_DistributedLoad/result
...
do
  ...
  --mount type=bind,source=${result_path},target=${jmeter_resultpath} \
	--rm labdocker/jmeter:lab032023 -n -s \
...
docker run --name $CLIENT_IP \
  ...
  --rm labdocker/jmeter:lab032023 -n -X \
